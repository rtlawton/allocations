...
#
SQL <- paste("select journal.id as id, dt, ac, ac_amount, comm from journal inner join funds on journal.fu = funds.id WHERE protocol = '", protocol, "' AND (ac < 40 OR ac > 47) AND journal.id > 3471", sep="")
...
#
dbWriteTable(mydb, value = final_tree, name = paste("Model_",protocol, sep=""), append = F )
#
...
#
SQL <- "select matchstring, ac from Bankmatch"
rs <- dbSendQuery(mydb, SQL)
qdata <- dbFetch(rs, n=-1)
